...
/***************************************************************************
	создание и обработка событий для окна Task Window
***************************************************************************/
	
predicates
 task_win_eh : EHANDLER	%обработчик событий окна
constants
 steps_cnt = 64
clauses
	% Волк, коза, капуста
	task_win_eh(_Win, e_Menu(id_lab_5_volk_koza_kapusta, _ShiftCtlAlt), 0):-
		test_dfs(X), % вызов программы
	...
	task_win_eh(_Win, e_Menu(id_lab_4_tick_tack_toe, _ShiftCtlAlt), 0):-
		dlg_tick_tack_toe_Create(_Win), !. % вызов программы

goal	%главный goal - создание окна
...
/**************************************************************************
	создание и обработка событий окна Крестики-нолики
**************************************************************************/
predicates
...
%события нажатия на кнопки
  dlg_tick_tack_toe_eh(_Win, e_Control(idc_1, _CtrlType, _CtrlWin, _CtlInfo),0):- !,
    Han = win_GetCtlHandle(_Win, idc_1), %получение дескриптора
    win_GetText(Han)="", win_SetText(Han,"X"), go(_Win), !. %если поле свободно, то поставить туда Х и передать ход компьютеру
  ...
  %ход компьютера
  go(_Win):-
  %получение обработчика кнопки и надписи
    ...
    win_SetText(Han1, Q1),	%установка новых надписей
    win_SetText(Han2, Q2),
    win_SetText(Han3, Q3),
    win_SetText(Han4, Q4),
    win_SetText(Han5, Q5),
  ...
  message(Res,_Win):-
    Res=1, dlg_MessageBox("Конец игры", "Вы выиграли", mesbox_iconInformation, 
    mesbox_buttonsOK, mesbox_defaultFirst, mesbox_suspendApplication),
    win_Destroy(_Win),!;
    
    Res=2, dlg_MessageBox("Конец игры", "Вы проиграли", mesbox_iconInformation, 
    mesbox_buttonsOK, mesbox_defaultFirst, mesbox_suspendApplication),
    win_Destroy(_Win),!;
    
    Res=3, dlg_MessageBox("Конец игры", "Ничья", mesbox_iconInformation, 
    mesbox_buttonsOK, mesbox_defaultFirst, mesbox_suspendApplication),
    win_Destroy(_Win),!;!.
